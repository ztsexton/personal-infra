...
echo ""
echo "4. Let the operator pick it up. The OnePasswordItem is already applied by"
echo "   Argo CD from git -- do not apply it by hand:"
echo "     ./scripts/secrets.sh resync"
echo "     ./scripts/secrets.sh status"
echo ""
echo "5. Zot reads the htpasswd file at startup, so restart it:"
echo "     ./scripts/staging.sh ssh kubectl rollout restart deployment/zot -n web"
echo ""
